...
| --- | --- | --- | --- |
| Account | In Progress | MindOFF 자체 가입·이메일 인증·로그인·재설정 화면, Cognito JWT, 토큰 갱신·로그아웃, 실제 가입·로그인 확인 | 토큰 갱신·로그아웃 운영 검증 |
...
| 영역 | 상태 | 비고 |
| --- | --- | --- |
| Cognito | In Progress | User Pool과 직접 인증 API 배포, 실제 이메일 인증·로그인 확인 | 토큰 갱신·로그아웃 운영 검증 |
